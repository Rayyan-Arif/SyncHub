...
export const removeMemberFromTeam = catchAsync(async(req, res, next) => {
    const {organization_id, team_id, member_id} = req.body;

    if(!(+team_id) || !(+member_id))
        throw new AppError("Please provide both team and user.", 400);

    //checking if member exists in this organization
    const userID = (await pool.query("SELECT 1 FROM organization_membership WHERE organization_id = $1 AND user_role = 'MEMBER' AND user_id = $2;", [+organization_id, member_id])).rows[0];
    ...
    await pool.query('DELETE FROM team_membership WHERE team_id = $1 AND team_member_id = $2;', [team_id, member_id]);
