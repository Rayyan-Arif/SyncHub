...
export const isTeamOfOrganization = catchAsync(async(req, res, next) => {
    const {organization_id, team_id} = req.body ?? req.params;
    
    const isTeamValid = (await pool.query("SELECT team_id FROM team WHERE team_id = $1 AND organization_id = $2 AND manager_id = $3;", [team_id, organization_id, req.user.user_id])).rowCount;
